function createMovieCard(movie) {
    ...
    return `
        <div class="bg-gray-800 rounded-lg overflow-hidden cursor-pointer transform hover:scale-105 transition-transform" onclick="bookMovie('${movie.id}', '${movie.title}')">
            <img src="${posterUrl}" alt="${movie.title}" class="w-full h-72 object-cover">
            <div class="p-4">
                <div class="text-lg font-semibold mb-2">${movie.title}</div>
                <div class="text-gray-400 text-sm">${new Date(movie.release_date).toLocaleDateString()}</div>
            </div>
        </div>
    `;
}
